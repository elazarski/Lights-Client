		...
		if (strcmp(name, "numberInputs") == 0) {
			...
			*numOutputTracks = atoi((char *)current->children->content);
			...
			while (inputTrack != NULL) {
				char *trackName = (char *)inputTrack->name;

				if (strcmp(trackName, "numTracks") == 0) {
					numTracks = atoi((char *)inputTrack->children->content);
					inputTracks.push_back(vector<int>(numTracks));

				} else if (strcmp(trackName, "track") == 0) {
			...
			while (outputTrack != NULL) {
				char *trackName = (char *)outputTrack->name;
...
vector<Event> parsePart(xmlNodePtr parent, int track) {
	...
	while (part != NULL) {
		...
		if (strcmp(name, "measure") == 0) {
			...
			// append measure to partVector
			for (unsigned int i = 0; i < measure.size(); i++) {
				Event current = measure.at(i);

				/*	NOTE EVENTS
				* normal note:		type = NOTE, time = duration, channel = 0
				*/

				/*  CHORD EVENTS
				 * note in chord:	type = CHORD, time = duration, channel = 1
				 */
				 ...
				 * time signature:	type = META, channel = 5, num = time signature
				 */

				/*  TIE EVENTS
				 * tied note:		type = TIE, time = duration to be appended, channel = 0
				 * * tied note at same time as another note:	type = TIE, time = duration to be appended, channel = 1
				 */

				if (current.type == NOTE) {
					// needn't do anything
					// merely append and adjust variables

					partVector.push_back(current);

					// change lastTime if needed (chord)
					if (partVector.size() > 1 && partVector.at(partEnd - 1).type == CHORD) {
						lastTime = lastTime + lastDur * timeSig/divisions/4;
					}

					partVector.at(partEnd).time = lastTime;

					Event e;

					// check if we are in a chord
					// if we are, do not update lastTime
					if (measure.size() > i + 1 && measure.at(i + 1).type != CHORD) {
						// not a chord
						lastTime = lastTime + current.time * timeSig/divisions/4;
					} else {
						partVector.at(partEnd).type = CHORD;
					}

					partEnd++;
				} else if (current.type == CHORD) {

					// check if merely a tied note ringing
					// at the same time that another is hit
	...
	while (measure != NULL) {
		...
		if (strcmp(name, "attributes") == 0) {
			...
			while (attributes != NULL) {
				char *attrName = (char *)attributes->name;

				if (strcmp(attrName, "divisions") == 0) {
				...
				} else if (strcmp(attrName, "time") == 0) {
					xmlNodePtr time = attributes->children;

					while (time != NULL) {
						char *timeName = (char *)time->name;

						if (strcmp(timeName, "beat-type") == 0) {
							Event bt;
							bt.type = META;
			...
			data.push_back(parseNote(measure));
		}
...
Event parseNote(xmlNodePtr parent) {
	...
	while (note != NULL) {
		char *name = (char *)note->name;

		if (strcmp(name, "pitch") == 0) {
			e.num = parsePitch(note);
			e.type = NOTE;
			...
			e.time = atoi((char *)note->children->content);

		} else if (strcmp(name, "rest") == 0) {
			e.type = META;
			e.channel = 4;
		} else if (strcmp(name, "tie") == 0) {
			char *tieChar = (char *)xmlGetProp(note, (xmlChar *)"type");

			if (strcmp(tieChar, "stop") == 0) {
				e.type = TIE;
			}
...
int parsePitch(xmlNodePtr parent) {
	...
	case 'B':
		...
		break;
	default:
		fprintf(stderr, "Error reading step node in pitch\n");
	}

	num = num + atoi((char *)octave) * 12;

	if (alter != NULL) {
		num = num + atoi((char *)alter);
	}
